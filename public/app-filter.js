...
function isPullRequestVisible(prElement, pullRequestData, assignee, reviewer, sprint, fixVersion, sync, ready) {
    ...
    const assigneeMatch = assignee === "Show all" || (() => {
        ...
        return jiraIssueKeys.some(issueKey => {
            ...
            return issue && issue.fields.assignee && issue.fields.assignee.displayName === assignee;
        });
    })();
...
function updatePullRequestStyle(prElement, pullRequestData, assignee, reviewer) {
    ...
    // Check if any Jira issue for this PR is assigned to the filtered assignee
