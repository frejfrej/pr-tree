...
* Version 2.7.0
    ...
    * Git's rule on the two diffs: overlapping or adjacent changes conflict unless they are identical, the same resulting file on both sides merges cleanly (binary files included), line endings count, a file deleted on one side and changed on the other conflicts, and so do different renames of one file and two files renamed onto one path; a diff that cannot be read completely reports the pull request not checked rather than clean (or, when conflicts are already certain, SYNC with the files that could not be checked named in the tooltip)
    * Two to four Bitbucket requests per pull request instead of about ten, so the SYNC load is affordable on the largest project; two loads of one project at once share one computation; the results are kept for good in `sync-cache.json` (git-ignored), reused after a restart, and only the pull requests whose commits moved are computed again
