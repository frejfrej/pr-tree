...
**Branches:** one per stage, stacked: `claude/sync-statuses-module` (from `master`), `claude/fixture-route-tests`, `claude/small-coverage-gaps`. Never `git add` `config.js`, `config.js.test` or `.claude/`; add files by name.
