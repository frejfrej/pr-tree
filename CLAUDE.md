...
- `atlassianFetch` (the fixture-mode guard, the pause after a 429, the network failure message with its cause and the URL, `shortMessage` without the URL) stays here; `RateLimitError` and `failureReason` are imported from `atlassian-fetch.mjs`
...
**sync-statuses.mjs**
- `createSyncStatuses({ fetch, workspace, bbAuth, log: { error, performance }, syncCache, timeoutMs = 30000, maxConcurrent = 4 })`: the SYNC computation of a server, one limiter and one cache; nothing here talks to Bitbucket by itself (the server passes `atlassianFetch`, the tests a fake answering by URL); returns `computeConflicts(repoName, spec)` and `computeSyncStatuses(projectName, pullRequests)`
